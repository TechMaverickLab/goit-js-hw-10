...
const{Axios:eh,AxiosError:ep,CanceledError:ed,isCancel:ey,CancelToken:eg,VERSION:em,all:eb,Cancel:eE,isAxiosError:ew,spread:ev,toFormData:eA,AxiosHeaders:eO,HttpStatusCode:eS,formToJSON:eR,getAdapter:eT,mergeConfig:eB}=ec;ec.defaults.headers.common["x-api-key"]="live_IlxTCE8YSC24414EQo40TXBc0MGBSsE5sxGaBHATmcJqGsIpvmZ3GrgtJpBnSveg";const eU=async()=>{try{let t=await ec.get("https://api.thecatapi.com/v1/breeds");return t.data}catch(t){throw console.error("Error fetching breeds",t),t}},eC=async t=>{try{let e=await ec.get(`https://api.thecatapi.com/v1/images/search?breed_ids=${t}`);return e.data[0]}catch(t){throw console.error("Error fetching cat by breed",t),t}},eL=document.querySelector(".breed-select"),ex=document.querySelector(".loader"),ej=document.querySelector(".error"),eN=document.querySelector(".cat-info"),eP=document.querySelector(".cat-text"),e_=document.querySelector(".modal"),eF=document.getElementById("img01"),eI=document.getElementById("caption"),ek=document.querySelector(".close-modal");async function eD(){eL.classList.add("hidden"),eN.classList.add("hidden"),document.body.tabIndex=-1,document.body.focus();try{ex.classList.remove("hidden");let t=await eU();t.forEach(t=>{let e=document.createElement("option");e.value=t.id,e.textContent=t.name,eL.appendChild(e)}),eL.classList.remove("hidden")}catch(t){console.error("Failed to fetch breeds",t),ej.textContent="Error loading breeds. Please try again later.",ej.classList.remove("hidden")}finally{ex.classList.add("hidden")}}async function eM(t){eL.classList.add("hidden");try{ex.classList.remove("hidden"),eN.classList.add("hidden");let e=await eC(t),r=eN.querySelector(".cat-image");r&&r.remove();let n=document.createElement("img");n.src=e.url,n.alt=e.breeds[0].name,n.classList.add("cat-image"),eN.prepend(n),eP.innerHTML=`
      <h1>${e.breeds[0].name}</h1>
      <p>${e.breeds[0].description}</p>
      <p>${e.breeds[0].temperament}</p>
    `,eN.classList.remove("hidden"),eL.classList.remove("hidden")}catch(t){console.error("Failed to fetch cat by breed",t),ej.textContent="Error loading cat details. Please try again later.",ej.classList.remove("hidden")}finally{ex.classList.add("hidden")}}function eq(t){t.target.classList.contains("cat-image")&&t.target.src?"flex"!==e_.style.display&&(e_.style.display="flex",eF.src=t.target.src,eI.innerHTML=t.target.alt||"",eI.style.display=t.target.alt?"block":"none"):(t.target===e_||t.target===ek)&&(e_.style.display="none")}function ez(t){"Escape"===t.key&&"flex"===e_.style.display&&(e_.style.display="none")}document.addEventListener("DOMContentLoaded",async()=>{await eD(),eL.addEventListener("change",async()=>{let t=eL.value;await eM(t)}),document.addEventListener("click",eq),document.addEventListener("keydown",ez)});//# sourceMappingURL=index.406f50b4.js.map

//# sourceMappingURL=index.406f50b4.js.map
